    ...
    <div class="flex flex-wrap items-center">
      <div class="relative w-full px-4 max-w-full flex-grow flex-1">
        <h3 class="font-semibold text-lg {color === 'light' ? 'text-blueGray-700' : 'text-white'}">
          ...
          {currentBot.username} - Console Log
        </h3>
      </div>
      <BotCommandInput {id}/>
      <a
        ...
        {#if logHistory == undefined}
          <tr class="animate-pulse bg-blueGray-200">
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left flex items-center">
              <span class="ml-3 font-bold {color === 'light' ? 'btext-blueGray-600' : 'text-whit'}"> Loading console history... </span>
            </td>
            <td></td>
            <td></td>
          </tr>
        {:else if logHistory.length == 0}
          <tr>
            <th colspan="2" class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left flex items-center">
              <span class="ml-3 font-bold {color === 'light' ? 'btext-blueGray-600' : 'text-whit'}"> No console history yet. </span>
            </th>
          </tr>
        {:else}
          {#each logHistory as { timestamp, level, message }, i}
            <tr>
              ...
              <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-1"> {level} </td>
